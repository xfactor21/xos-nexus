...
const STORAGE_KEY = 'xos-studio-v1';
...
/**
 * DESIGN STUDIO — Step 4: ported 1:1 from xos-prototype.html (infinite
 * canvas, pan/pinch/zoom, pen/frame/sticky tools) and extended per the
 * Feature Uplift notes: rectangle/circle/arrow shape tools, image
 * paste/upload, a real layers panel (drag-to-reorder + visibility
 * toggles), multi-select + group move, alignment guides/snapping,
 * comment pins, and undo/redo. Position/size/type persist to
 * localStorage as a stand-in for `nodes.metadata` jsonb until Step 1
 * (Auth) lands a real owner_id to write against Supabase with — see the
 * session log for that hand-off note.
 */
export default function Studio({ active }: { active: boolean }) {
  ...
  const [snap, setSnap] = useState<StudioSnapshot>(loadSnapshot);
  ...
  useEffect(() => {
    if (!initDone) return;
    const t = setTimeout(() => localStorage.setItem(STORAGE_KEY, JSON.stringify(snap)), 300);
  ...
  return (
    <section className={`room ${active ? 'on' : ''}`} id="r-studio" style={{ maxWidth: 'none', padding: '56px 8px 90px' }}>
      ...
      <div className="rsub" style={{ paddingLeft: 4 }}>
        INFINITE CANVAS · DRAG EMPTY SPACE TO PAN · SHIFT+DRAG TO MARQUEE-SELECT · PINCH/SCROLL TO ZOOM · EVERYTHING FEEDS THE CORE
      </div>
      <div
        id="stWrap"
        ref={wrapRef}
